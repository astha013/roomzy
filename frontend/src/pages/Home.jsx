...
const NEW_FEATURES = [
  { icon: '⭐', title: 'Rich Rating System', desc: 'Rate roommates on Cleanliness, Behavior, Cooperation & Punctuality for transparent trust.' },
  { icon: '🏠', title: 'Room Listings', desc: 'Post or browse available rooms with photos, rent, facilities & location on a map.' },
  { icon: '🛡️', title: 'Fraud Detection', desc: 'AI spam filtering, fake profile detection & real-time report system keep you safe.' },
  { icon: '🗺️', title: 'Location Search', desc: 'Find roommates and rooms near your college or office with GPS-based search.' },
  { icon: '🧠', title: 'Sentiment AI', desc: 'NLP analysis detects toxic language, spam & fraud attempts in real-time chat.' },
  { icon: '🎙️', title: 'Voice Search', desc: 'Search by speaking naturally — "Find rooms near college under ₹8000".' },
];
...
export default function Home() {
  ...
  return (
    <div>
        ...
        <div style={rightColStyle}>
          ...
          <div className="float-cards-container" style={{ position: 'relative', width: 360, height: 700, padding: '40px 0' }}>
            ...
            <FloatCard style={{ top: 20, left: 0, animationDelay: '0s' }}>
              <MiniMatchCard name="Aastha Kaushik" city="Pune" score={92} trust={78} intent="have_room_need_roommate" />
            </FloatCard>
            {/* Card 2 - Arjun (middle right) */}
            <FloatCard style={{ top: 210, right: 0, animationDelay: '1.4s' }}>
              <MiniMatchCard name="Ankit Dawar" city="Indore" score={95} trust={96} intent="looking_for_roommate" />
            </FloatCard>
            {/* Card 3 - Neha (bottom left) */}
            <FloatCard style={{ top: 380, left: 0, animationDelay: '2.8s' }}>
              <MiniMatchCard name="Harsh Solanki" city="Coimbatore" score={69} trust={55} intent="have_room_need_roommate" />
            </FloatCard>
            {/* Card 4 - Rahul (extra card bottom right) */}
            <FloatCard style={{ top: 550, right: 0, animationDelay: '3.5s' }}>
              <MiniMatchCard name="Astha Pawar" city="Mumbai" score={81} trust={70} intent="have_room_need_roommate" />
            </FloatCard>
          </div>
        </div>
      </section>
      ...
      {/* ── NEW FEATURES ───────────────────────────────────────────────── */}
      <section style={{ padding: 'clamp(3rem, 6vw, 5rem) clamp(1.5rem, 5vw, 4rem)', background: 'var(--clay)', color: 'white' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 900, marginBottom: '0.75rem' }}>
              What's New in Roomzy
            </h2>
            <p style={{ color: 'rgba(255,255,255,0.65)', maxWidth: 560, margin: '0 auto' }}>
              Powerful new features to make your roommate search safer, smarter and easier.
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            {NEW_FEATURES.map(({ icon, title, desc }) => (
              <div key={title} style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 16, padding: '1.5rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                <div style={{ fontSize: '2rem', marginBottom: '0.75rem' }}>{icon}</div>
                <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, marginBottom: '0.5rem', fontSize: '1rem' }}>{title}</h3>
                <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
