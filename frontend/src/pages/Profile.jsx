...
const CITIES = [
  ...
  'Patna', 'Meerut', 'Pimpri-Chinchwad', 'Hubli', 'Belgaum',
];
...
function ReviewsSection({ userId }) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 clamp(1rem, 5vw, 2rem) 3rem' }}>
      <h2 style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '1.15rem', marginBottom: '1.25rem', borderBottom: '1px solid var(--parchment-3)', paddingBottom: '0.75rem' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem 2rem', color: 'var(--clay-3)', background: 'var(--parchment-1)', borderRadius: 16, border: '1px dashed var(--parchment-3)' }}>
          <div style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>🏡</div>
          <p style={{ fontWeight: 600 }}>No reviews yet</p>
          <p style={{ fontSize: '0.82rem', marginTop: 4 }}>Reviews from matched roommates will appear here</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: '1fr 1fr' }}>
          {/* Averages summary card */}
          <div style={{ background: 'white', borderRadius: 16, padding: '1.25rem', border: '1px solid var(--parchment-3)', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', gridColumn: '1 / -1' }}>
            <p style={{ fontWeight: 800, fontSize: '0.8rem', color: 'var(--clay-3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '1rem' }}>Average Ratings ({reviews.length} review{reviews.length !== 1 ? 's' : ''})</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.75rem' }}>
              {DIMS.filter(d => averages[d.key] !== null && averages[d.key] !== undefined).map(d => (
                <div key={d.key} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--clay-2)' }}>{d.icon} {d.label}</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <StarDisplay value={averages[d.key]} />
                    <span style={{ fontSize: '0.78rem', fontWeight: 800, color: 'var(--terra)' }}>{averages[d.key]}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Individual reviews */}
          {reviews.map(r => (
            <div key={r._id} style={{ background: 'white', borderRadius: 16, padding: '1.25rem', border: '1px solid var(--parchment-3)', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.875rem' }}>
                {r.reviewer?.profilePhoto ? (
                  <img src={`${BASE_API}${r.reviewer.profilePhoto}`} alt="" style={{ width: 38, height: 38, borderRadius: '50%', objectFit: 'cover' }} />
                ) : (
                  <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--terra-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.9rem', color: 'var(--terra)' }}>
                    {(r.reviewer?.name || '?')[0].toUpperCase()}
                  </div>
                )}
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: '0.88rem' }}>{r.reviewer?.name || 'Anonymous'}</div>
                  <div style={{ fontSize: '0.68rem', color: 'var(--clay-3)' }}>
                    {r.stayDuration && `Lived together: ${r.stayDuration} · `}
                    {new Date(r.createdAt).toLocaleDateString([], { month: 'short', year: 'numeric' })}
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                  <StarDisplay value={r.ratings?.overall || 0} />
                  {r.wouldRecommend !== null && r.wouldRecommend !== undefined && (
                    <span style={{ fontSize: '0.65rem', fontWeight: 700, color: r.wouldRecommend ? '#22c55e' : '#ef4444' }}>
                      {r.wouldRecommend ? '👍 Recommended' : '👎 Not recommended'}
                    </span>
                  )}
                </div>
              </div>
              ...
              {/* Dimension ratings */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.35rem', marginBottom: r.comment ? '0.875rem' : 0 }}>
                {DIMS.filter(d => d.key !== 'overall' && r.ratings?.[d.key]).map(d => (
                  <div key={d.key} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem' }}>
                    <span>{d.icon}</span>
                    <span style={{ color: 'var(--clay-3)', flex: 1 }}>{d.label}</span>
                    <StarDisplay value={r.ratings[d.key]} size={10} />
                  </div>
                ))}
              </div>

              {r.comment && (
                <p style={{ fontSize: '0.82rem', color: 'var(--clay-2)', lineHeight: 1.6, fontStyle: 'italic', borderTop: '1px solid var(--parchment-3)', paddingTop: '0.75rem', margin: 0 }}>
                  "{r.comment}"
                </p>
              )}
            </div>
