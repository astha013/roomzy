...
api.interceptors.response.use(
  ...
  (err) => {
    if (err.response?.status === 401) {
      ...
      // FIX: Don't fire logout for auth endpoints — login/register handle their
      // own 401s (wrong password, unverified email, etc.). Only dispatch logout
      // when a previously-authenticated request is rejected (expired/invalid token).
...
export const matchApi = {
  getSuggestions: (filters = {}) => {
    ...
    return api.get(`/matches/suggestions?${params.toString()}`);
  },
...
export const chatApi = {
  ...
  start: (recipientId) => api.post('/chat/start', { recipientId }),
  deleteChat: (chatId) => api.delete(`/chat/${chatId}`),
  unsendMessage: (chatId, messageId) => api.delete(`/chat/${chatId}/messages/${messageId}`),
};
...
export const roomApi = {
  ...
  create:    (formData) => api.post('/rooms', formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
  update:    (id, formData) => api.put(`/rooms/${id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
  delete:    (id) => api.delete(`/rooms/${id}`),
  save:      (id) => api.post(`/rooms/${id}/save`),
};

// ══════════════════════════════════════════════════════════════════════════
// ADMIN FAKE PROFILES  — /api/admin/fake-profiles
// ══════════════════════════════════════════════════════════════════════════
export const fakeProfileApi = {
  scan: () => api.get('/admin/fake-profiles'),
};
